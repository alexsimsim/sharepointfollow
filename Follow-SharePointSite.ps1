...
<#
.SYNOPSIS
    Makes specified Azure users follow a SharePoint site.

.DESCRIPTION
    This script takes a list of Azure users, an optional AD group, and a SharePoint site,
    then makes those users follow the specified SharePoint site. Users can be provided
    directly via UserIds parameter or through an AD group via GroupId parameter.
...
# Script parameters (now optional since values are stored above)
param(
...
# Use parameter values if provided, otherwise use stored values
if ($TenantIDParam) { $TenantID = $TenantIDParam }
if ($ApplicationIdParam) { $ApplicationId = $ApplicationIdParam }
if ($ApplicationSecretParam) { $ApplicationSecret = $ApplicationSecretParam }
if ($SiteIdParam) { $SiteId = $SiteIdParam }
if ($UserIdsParam) { $UserIds = $UserIdsParam }
if ($GroupIdParam) { $GroupId = $GroupIdParam }
if ($SiteUrlParam) { $SiteUrl = $SiteUrlParam }

# Validate that either UserIds or GroupId is provided
if (-not $UserIds -and -not $GroupId) {
    Write-Error "Either UserIds or GroupId must be provided in the configuration section or as parameters."
    exit 1
}
